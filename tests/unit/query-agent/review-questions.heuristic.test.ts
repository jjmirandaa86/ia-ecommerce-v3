...
import {
  REVIEW_QUESTION_CASES,
  reviewQuestionStats,
} from "../../fixtures/review-questions";

describe("review question catalog", () => {
  it("contains exactly 100 cases with unique ids", () => {
    expect(REVIEW_QUESTION_CASES).toHaveLength(100);
    const ids = REVIEW_QUESTION_CASES.map((c) => c.id);
    expect(new Set(ids).size).toBe(100);
  });

  it("reports implemented vs backlog counts", () => {
    const stats = reviewQuestionStats();
  ...
  it("every implemented case (except no_match) uses a known intent", () => {
    for (const c of REVIEW_QUESTION_CASES.filter((x) => x.implemented)) {
...
describe("review heuristic — implemented (must pass)", () => {
  const cases = REVIEW_QUESTION_CASES.filter((c) => c.implemented);
...
describe("review heuristic — backlog (skipped until implemented: true)", () => {
  const cases = REVIEW_QUESTION_CASES.filter((c) => !c.implemented);
...
describe("review heuristic — backlog guard (flip implemented when green)", () => {
  const cases = REVIEW_QUESTION_CASES.filter((c) => !c.implemented);
